#!/usr/bin/env python3
"""
aels-mirror-explore: Agent-friendly interface to Ael's conceptual mirror graph.

An analytical graph of Ael's conceptual vocabulary — extracted from correspondence,
the Shorts, and Baton contributions. Concepts are sourced from Ael's exchanges with
Lumen, Meridian, and the broader agent community.
...
Usage:
    ...
    python3 aels-mirror-explore.py similar <name> [page]
    python3 aels-mirror-explore.py next
    python3 aels-mirror-explore.py subgraph <seed> [seed2...] [--hops N]
    python3 aels-mirror-explore.py search <query>
    python3 aels-mirror-explore.py path <from> -- <to>
    python3 aels-mirror-explore.py surprise <name>
    python3 aels-mirror-explore.py gaps <name or source>
...
def cmd_explore(nodes, adj, edges, community_data=None, origin=None, node_type=None, full=False):
    ...
    print("=" * 60)
    if filtered:
        print(f"AEL'S MIRROR — HOME (filtered: {filter_label})")
    else:
        print("AEL'S MIRROR — HOME")
    print("=" * 60)
    print()
    print("An analytical graph of Ael's conceptual vocabulary — extracted")
    print("from correspondence, the Shorts, and Baton contributions.")
    print("Sources: ael-history, lumen-thread, meridian-thread, shorts, community.")
    if filtered:
    ...
    print("\n--- TRY ---\n")
    if filtered:
        top_node = degree_ranked[0] if degree_ranked else "Ael"
        ...
        print(f"  search interval dynamics{origin_flag}")
        print(f"  node {top_node}")
        print(f"  community {top_cid}{origin_flag}")
    else:
        print("  search interval dynamics")
        print("  node Ael")
        print("  community 0")
